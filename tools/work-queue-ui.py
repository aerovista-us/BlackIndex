...
def main() -> int:
    ...
    lineage_rows="".join(f"<tr><td><code>{esc(x.get('doc_a'))}</code></td><td><code>{esc(x.get('doc_b'))}</code></td><td>{esc(x.get('cooccurrence_files'))}</td><td>{'<br>'.join(esc(f) for f in x.get('files',[]))}</td></tr>" for x in lineage_pairs[:100])
...
<header><h1>BlackIndex · Work Queue</h1><div class="muted">Unresolved work states only. Queue membership is not evidence of importance, wrongdoing, or truth.</div><div class="nav"><a href="/blackindex-dashboard.html">Evidence Map</a><a href="/source-lineage.html">Source Lineage</a></div></header>
...
<h2>Research cross-references awaiting lineage review</h2><table><thead><tr><th>Document A</th><th>Document B</th><th>Research files</th><th>Where recognized</th></tr></thead><tbody>{lineage_rows or '<tr><td colspan="4">No unresolved research cross-reference pairs.</td></tr>'}</tbody></table>
