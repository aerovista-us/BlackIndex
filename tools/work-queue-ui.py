...
def main() -> int:
    ...
    mismatch_rows="".join(f"<tr><td><code>{esc(d.get('doc_id'))}</code></td><td>{esc(d.get('metadata_evidence_status'))}</td><td>{esc(d.get('extraction_state'))}</td><td>{esc(d.get('finding'))}</td><td>{esc(d.get('status'))}</td></tr>" for d in review_mismatches[:100])
...
<h2>Metadata / extraction review-state drift</h2><p class="muted">A non-stub extraction can indicate review activity while metadata still says unreviewed. This is a synchronization queue only; no evidence status is changed automatically.</p><table><thead><tr><th>Document</th><th>Metadata status</th><th>Extraction state</th><th>Finding</th><th>Status</th></tr></thead><tbody>{mismatch_rows or '<tr><td colspan="5">No review-state mismatches detected.</td></tr>'}</tbody></table>
...
<h2>Documents still marked unreviewed</h2><p class="muted">Use the review-state drift table above to distinguish likely status lag from untouched neutral stubs.</p><table><thead><tr><th>Document</th><th>Title</th><th>Source</th><th>Collection</th></tr></thead><tbody>{unreviewed_rows or '<tr><td colspan="4">No metadata records currently marked unreviewed.</td></tr>'}</tbody></table>
</main>'''
