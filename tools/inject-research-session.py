...
(function(){
 ...
 function syncPinButton(){const id=currentId();const tools=document.querySelector('.bi-record-tools');if(!id||!tools)return;let b=tools.querySelector('[data-bi-pin]');if(!b){b=document.createElement('button');b.type='button';b.dataset.biPin='1';b.onclick=()=>togglePin(currentId());tools.insertBefore(b,tools.firstChild)}const active=read(KEY_PINS).includes(id);const label=active?'Unpin record':'Pin record';if(b.textContent!==label)b.textContent=label;if(b.classList.contains('bi-pin-active')!==active)b.classList.toggle('bi-pin-active',active)}
 let last=null;function observe(){const id=currentId();if(id&&id!==last){last=id;touchRecent(id)}syncPinButton()}
 let queued=false;function scheduleObserve(){if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;observe()})}
 const view=document.getElementById('view');if(view)new MutationObserver(scheduleObserve).observe(view,{childList:true,subtree:true});window.addEventListener('hashchange',()=>setTimeout(scheduleObserve,0));
