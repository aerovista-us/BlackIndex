...
if [[ -n "$ECDOC" ]]; then
  python3 "$EVIDENCE" --root "$ROOT" missing-evidence "$ECDOC" \
...
PYTHONWARNINGS="ignore::SyntaxWarning" python3 "$EVIDENCE" --root "$ROOT" dashboard
python3 "$ROOT/tools/blackindex.py" --root "$ROOT" verify
python3 "$EVIDENCE" --root "$ROOT" publish --push

echo "9/11 underlying-record pass complete."
